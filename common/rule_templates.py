...
class Template(object):
    def __init__(self, head, predicates, rule_index, max_var=4):
        self.head = head
    ...
    def gen_helper(self, pos_b, neg_b):
        ...
        for prod in product(*product_list):
            # print('prod', prod)
            values = [prod[sorted_type[t]][ti] for t, ti in zip(variable_types, type_i)]
            # print('values', values)
            i = 0
            ...
            if self.safe(head, body, variable_types):
                ...
                rules.append(Rule(head, body, rule_variables, self.rule_index.get_and_inc()))
    ...
    def gen_type_combinations(self, count):
        ...
        variable_combinations = filter(self.no_gaps,
                                       combinations_with_replacement(variables, count))
